...
var ecdsaSig = require('wntcore/signature.js');
...
setTimeout(function(){
	readKeys(function(mnemonic_phrase, passphrase, deviceTempPrivKey, devicePrevTempPrivKey){
		...
		readSingleWallet(function(wallet){
			// global
			wallet_id = wallet;
			require('wntcore/wallet.js'); // we don't need any of its functions but it listens for hub/* messages
			var device = require('wntcore/device.js');
			...
			db.query("SELECT 1 FROM extended_pubkeys WHERE device_address=?", [my_device_address], function(rows){
				...
				if (conf.bLight){
					var light_wallet = require('wntcore/light_wallet.js');
